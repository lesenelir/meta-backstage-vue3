...
// http 请求拦截：是在axios请求发出之前给每一个接口携带token去后端校验身份
...
instance.interceptors.response.use(
  ...
  error => {
    if (error.response) {
      const ERRS = error.response.status // 状态码
      const MSG = error.response.data.msg.msg
      const errdata = ERRS === 401 ? MSG : '服务器发生错误'
      switch (ERRS) {
        case 401:
          // 没有访问权限，token过期，没有携带token请求，token错误
          ElMessageBox.alert(errdata, 'Tips', {
